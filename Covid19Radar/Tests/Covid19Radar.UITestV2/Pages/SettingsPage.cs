...
namespace CovidRadar.UITestV2
{
    ...
    public class SettingsPage : BasePage
    {
        ...
        ***********/
        ...
        /// GitHubボタン.
        /// </summary>
        private static readonly string OpenGitHub = "OpenGitHub";

        private readonly Query backBtn;
        private readonly Query openMenuPage;
        private readonly Query tapSyokika;
        private readonly Query tapGitHub;
        private readonly Query tapLicense;
        ...
        public SettingsPage()
        {
            if (OnAndroid)
            {
                ...
                tapGitHub = x => x.Marked(SettingsPageTitle).Class(ButtonRenderer).Index(2); // ソースコードを入手
                tapLicense = x => x.Marked(SettingsPageTitle).Class(ButtonRenderer).Index(3); // ライセンス表記
            }

            if (OniOS)
            {
                backBtn = x => x.Class(UIButton).Index(4); // 戻るボタン
                openMenuPage = x => x.Class(UIButton).Index(3); // ハンバーガーメニュー
                tapSyokika = x => x.Marked(SettingsPageTitle).Class(UIButton).Index(1); // アプリ初期化
                tapGitHub = x => x.Marked(SettingsPageTitle).Class(UIButton).Index(2); // ソースコードを入手
                tapLicense = x => x.Marked(SettingsPageTitle).Class(UIButton).Index(3); // ライセンス表記
            }
        }
        ...
        /// 「ソースコードを入手」ボタンを押下する.
        /// </summary>
        public void TapGitHub()
        {
            app.ScrollDownTo(OpenGitHub, SettingsPageTitleScrollView);
            app.Tap(tapGitHub);
        }
